...
const Components = {
  ...
  createTaskCard(item, stationNumber = null, priority = 1) {
    ...
    checkbox.addEventListener('click', (e) => {
      ...
      const station = stationNumber || 1;
      const currentlyCompleted = Storage.isReviewCompleted(item.id, station, currentDateStr);
      ...
      setTimeout(() => {
        if (currentlyCompleted) {
          Storage.unmarkReviewComplete(item.id, station, currentDateStr);
        } else {
          Storage.markReviewComplete(item.id, station, currentDateStr);
        }
